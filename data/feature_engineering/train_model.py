...
def predict_match(home: str, away: str, models: dict,
    ...
    # Squad-metric nudge — all 18 features contribute.
    # first_xi_value: best starting 11 by market value; more predictive than
    #   total_squad_value because expensive bench players don't play.
    # club_linkage_score: 3+ players from one club → chemistry/familiarity bonus.
    # goals_per_player: squad international goal-scoring pedigree.
    # star_reliance_gini: high = fragile when star underperforms (a-h sign).
    # pct_elite_league: most discriminating vs CONCACAF/weak-league ELO inflation.
    squad_adv = (
        0.17 * np.tanh((h["first_xi_value"]       - a["first_xi_value"])      / 8e7)  # starting XI quality — highest weight
        ...
        + 0.08 * np.tanh((h["total_squad_value"]  - a["total_squad_value"])   / 1e8)  # full squad depth (bench)
        + 0.07 * np.tanh((h["star_player_value"]  - a["star_player_value"])   / 8e7)  # star quality gap
        + 0.06 * np.tanh((h["age_peak_score"]     - a["age_peak_score"])      * 5)    # peak age profile
        + 0.06 * np.tanh((h["depth_def"]          - a["depth_def"])           * 8)    # defensive depth
        + 0.05 * np.tanh((h["depth_att"]          - a["depth_att"])           * 7)    # attacking depth
        + 0.04 * np.tanh((h["depth_mid"]          - a["depth_mid"])           * 7)    # midfield depth
        + 0.04 * np.tanh((a["star_reliance_gini"] - h["star_reliance_gini"])  * 4)    # star fragility
        + 0.04 * np.tanh((a["total_club_minutes"] - h["total_club_minutes"])  / 1e4)  # burnout/fatigue
        + 0.03 * np.tanh((h["club_linkage_score"] - a["club_linkage_score"])  * 8)    # club chemistry
        + 0.03 * (float(h["has_elite_pedigree"])   - float(a["has_elite_pedigree"]))  # manager pedigree
        + 0.03 * np.tanh((h["depth_gk"]           - a["depth_gk"])           * 4)    # GK depth
        + 0.02 * np.tanh((h["goals_per_player"]   - a["goals_per_player"])    / 3)    # goal-scoring pedigree
        + 0.01 * np.tanh((h["tenure_days"]         - a["tenure_days"])        / 500)  # manager stability
